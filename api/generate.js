...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'latex') {
        prompt = `Đóng vai là một chuyên gia soạn thảo văn bản LaTeX. Hãy chuyển đổi toàn bộ nội dung từ tài liệu/tệp tin đính kèm dưới đây sang mã LaTeX hoàn chỉnh (từ \\documentclass đến \\end{document}). Tôi muốn định dạng, phông chữ và cách trình bày mang phong cách chuẩn học thuật, giống với các tài liệu hướng dẫn kỹ thuật chuyên nghiệp.
...
- Ngôn ngữ & Phông chữ: Hỗ trợ tiếng Việt chuẩn (\\usepackage[utf8]{inputenc}, \\usepackage[T5]{fontenc}, \\usepackage[vietnamese]{babel}). Bắt buộc sử dụng gói \\usepackage{lmodern} để lấy font chữ Latin Modern đặc trưng của LaTeX, tuyệt đối không dùng Times New Roman.
...
Nội dung văn bản bổ sung (nếu có): ${input || "Không có"}`;
        ...
        prompt = `Đóng vai một lập trình viên Front-end. Tôi cần bạn viết một file HTML tĩnh duy nhất (bao gồm cả CSS và JavaScript trong một file) để tạo ra một trò chơi trắc nghiệm giao diện hiện đại phục vụ giảng dạy dựa trên tài liệu đính kèm. Dưới cùng của trò chơi, hãy thêm một dòng chữ nhỏ: 'Thiết kế bởi DETOZ'.
...
3. Màn hình Kết quả:
   - Chỉ xuất hiện khi học sinh đã hoàn thành xong toàn bộ các câu hỏi.
...
- Khi trả lời sai: Nút đổi màu đỏ, và nút bị rung lắc. Tự động chuyển câu sau 1 giây.
